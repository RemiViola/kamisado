:-module(ia_mathieu_alphabeta,[jouer_mat_ab/0]).
...
goodenough( [], _, _, Pos, Val, Pos, Val, Iteration):-!.

goodenough(_, Alpha, Beta, Pos, Val, Pos, Val, Iteration) :-   % Pos0 better than Pos1
    min_to_move(Pos),                         % MIN to move in Pos0
    Val > Beta, !                             % MAX prefers the greater value
    ;
    max_to_move(Pos),                         % MAX to move in Pos0
    Val < Alpha, !.                            % MIN prefers the lesser value
